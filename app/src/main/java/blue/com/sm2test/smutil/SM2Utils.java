package blue.com.sm2test.smutil;
...
public class SM2Utils {
	 ...
	 * @return
	 */
	public static boolean verifySign(byte[] publicKey, byte[] sourceData,
			byte[] signData) {
		String userId = "1234567812345678";
	...
	public static void main(String[] args) throws Exception {
		System.err.print("hello it is sm2");
		System.out.println("");
		String plainText = "message digest ，谁开的房间了但是开发商浪蝶狂蜂索拉卡的交付但是哈哈哈哈";
		byte[] sourceData = plainText.getBytes();//转换到了字节
		...
		System.out.println("prikS: " + prikS);//将秘钥通过64位的程序进行了改变
		System.out.println("");

		// 国密规范测试用户ID
		String userId = "yuhonqin@163.com";

		System.out.println("ID: " + Util.getHexString(userId.getBytes()));//将用户转到了用户id
		System.out.println("");

		System.out.println("签名: ");
		byte[] c = SM2Utils.sign(userId.getBytes(),
				Base64.decode(prikS.getBytes()), sourceData);
		System.out.println("此时的sm2 的sign: " + Util.getHexString(c));
		System.out.println("");
		...
		boolean vs = SM2Utils.verifySign(userId.getBytes(),
				Base64.decode(pubkS.getBytes()), sourceData, c);//验签的时候竟然需要原来的数据
		System.out.println("验签结果: " + vs);
		System.out.println("");

		System.out.println("加密: ");
		byte[] cipherText = SM2Utils.encrypt(Base64.decode(pubkS.getBytes()),
				sourceData);
		System.out.println(new String(Base64.encode(cipherText)));
		System.out.println("");

		System.out.println("解密: ");
		plainText = new String(SM2Utils.decrypt(
				Base64.decode(prikS.getBytes()), cipherText));
		System.out.println(plainText);

	}

	public static void sm2ResultTest(){
		Log.e(TAG, "sm2ResultTest: " );
		System.err.print("hello it is sm2");
		System.out.println("");
		String plainText = "message digest ，谁开的房间了但是开发商浪蝶狂蜂索拉卡的交付但是哈哈哈哈";
		byte[] sourceData = plainText.getBytes();//转换到了字节
	...
	public static Sm2KeyPair generateKeyPair() {
		...
		// ecpriv.getD().toString(16).toUpperCase());
		// System.out.println("公钥: " +
		// ecpub.getQ().getX().toBigInteger().toString(16).toUpperCase() +
		...
		System.arraycopy(bigNumArray, bigNumArray[0] == 0 ? 1 : 0, priKey, 0,
				32);
		System.arraycopy(ecpub.getQ().getEncoded(), 1, pubKey, 0, 64);

		// System.out.println("私钥bigNumArray: " +
		// Util.getHexString(bigNumArray));
	...
	public static void main() {
		...
		System.err.println("beginstart: " +plainText);
		// 国密规范测试私钥
		String prik = "444E6EA3EE0C7E0AAA5EE5C6BBC7A2D8DE3FB3FA990AD470232D07FB445F92D7";
	...
	public static void Sm2Test() {
		...
		plainText = new String(
				SM2Utils.decrypt(keyPair.getPriKey(), cipherText));
		System.out.println(plainText);
	}
}
